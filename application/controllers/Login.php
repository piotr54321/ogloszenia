...
class Login extends AC_Controller {
    ...
    public function index(){
        ...
        if($this->input->post('submit') != null){
            $this->form_validation->set_rules(
                'username', 'username', 'trim|required|xss_clean',
                array(
            ...
            if(!$this->form_validation->run()){
                ...
                if($this->loginmodel->checkPass($dataFromForm)){
                    ...
                    $this->session->set_userdata($this->loginmodel->sessionUserData($dataFromForm['username']));
                    redirect('/');
                }else{
    ...
    public function register(){
        ...
        if($this->input->post('submit') != null){
            $this->form_validation->set_rules(
                'username', 'username', 'trim|required|xss_clean|min_length[4]|max_length[30]|is_unique[users.username]',
                array(
                    'required'=>'Nie wprowadziłeś nazwy użytkownika',
            ...
            $this->form_validation->set_rules('password', 'paswword', 'trim|required|xss_clean|min_length[6]',
                array(
                    'required'=>'Nie wprowadziłeś hasła',
                    'min_length'=>'Hasło powinno mieć przynajmniej 6 znaków'
                )
            );
            $this->form_validation->set_rules('password2', 'paswword2', 'trim|required|xss_clean|matches[password]',
                array(
            ...
            $this->form_validation->set_rules('email', 'email', 'trim|required|xss_clean|valid_email|is_unique[users.email]',
                array(
                    ...
                    'valid_email'=>'Wprowadzony adres email jest nieprawidłowy',
